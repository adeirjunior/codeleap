...
export default function CareerCard({ data }: CareerCardProp) {
  ...
  const dateNow = new Date(created_datetime)
  ...
  function timeAgo(dateNow: Date): string {
    const diferenca: number = diffHour(dateNow);
    ...
    <div className="border-[#999999] border rounded-2xl overflow-hidden">
      ...
      <div className="p-4">
        <div className="flex justify-between items-center mb-1">
          ...
          <p className="text-[#777777] ">{timeAgo(dateNow)}</p>
        </div>
        <p>{content}</p>
      </div>
    </div>
